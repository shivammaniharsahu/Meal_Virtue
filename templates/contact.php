...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if (empty($nm)) { array_push($errors, "Name is required"); }
  if (empty($em)) { array_push($errors, "Email is required"); }
  if (empty($sb)) { array_push($errors, "Subject is required"); }
  if (empty($me)) { array_push($errors, "Message is required"); } 
  ...
  if (count($errors) == 0) 
  {
      ...
      echo 'alert("Successfully response accepted"); location.href="contact.php"';
      echo '</script>'; 
  }
  else
  {
